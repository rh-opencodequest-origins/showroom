...
As you can see, it looks like a **SuperHero** has introduced a bad dependency in our code! 
...
IMPORTANT: Remove the dependency as it's not needed in our code, commit and push your changes !

== Deploy the Hero microservice after fixing the issue
...
You can add `/api/heroes/hello`, `/api/heroes/random`, or `/api/heroes` to the URL in your browser to attack the good endpoints and check your work
