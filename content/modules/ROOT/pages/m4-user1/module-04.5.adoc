...
Since the beginning of the workshop, we have used GitOps mechanisms thanks to ArgoCD. As a developer, it would be nice to get an overview of the objects we manage through our development environment without leaving our new hometown. The Internal Developer Portal!
...
. Click here : https://backstage-rhdh-{team-id}.{openshift_cluster_ingress_domain}/catalog/default/component/dev{team-id}-hero
...
The Super Heroes microservice stores superheroes, with their names, powers, and so on.
...
----
wget https://gitlab-gitlab.{openshift_cluster_ingress_domain}/pe{team-id}/fullcodebase/-/raw/main/hero-service.zip -O hero-service.zip && unzip -o hero-service.zip && rm hero-service.zip
----
...
Look at the `pom.xml`
You will find here the import of the Quarkus BOM (_bill of materials_), allowing you to omit the version of the different Quarkus dependencies.
...
You can use either the `quarkus` CLI or the `Apache Maven` command-line interface in a Terminal.
...
At some point, a pop-up asking to open the Quarkus dev will be shown:
...
Quarkus ships with a Developer UI, which is available in dev mode at the/q/dev-ui path by default.
...
You can run the tests with `./mvnw test` in a new Terminal.
...
#When you start Quarkus in dev mode, down at the bottom of the screen, you should see the following#:
...
You should see the status change at the bottom of the screen as they are running, and it should finish with
...
For our workshop, we need to return a random hero.
...
As Quarkus supports the automatic provisioning of unconfigured services in development and test mode, we don't need to configure anything regarding the database access at the moment. 
...
Look at the import.sql file located here `/src/resources/import.sql`. Around 500 heroes have been loaded in the database.

Now check the method findRandom defined above to confirm you can get random heroes! 
