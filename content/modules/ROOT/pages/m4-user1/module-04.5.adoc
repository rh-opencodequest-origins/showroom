...
```bash
wget https://gitlab-gitlab.{openshift_cluster_ingress_domain}/pe{team-id}/fullcodebase/-/raw/main/hero-service.zip?ref_type=heads -O hero-service.zip && unzip -o hero-service.zip && rm hero-service.zip
```
...
It is located under #`src/main/java/io/quarkus/workshop/hero`#
