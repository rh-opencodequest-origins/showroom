...
. Access your https://backstage-rhdh-{team-id}.{openshift_cluster_ingress_domain}/[{product_name_rhdh} instance^, window="rhdh"]. If prompted login using `{pe-username}` / `{common_password}`
...
----
https://gitlab-gitlab.{openshift_cluster_ingress_domain}/pe{team-id}/parasol-store-dev-template/-/blob/main/template.yaml
----
