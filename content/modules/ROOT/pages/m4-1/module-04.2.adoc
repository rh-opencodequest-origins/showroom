...
* *import-existing-api* - Available in the https://gitlab-gitlab.{openshift_cluster_ingress_domain}/pe{team-id}/import-existing-api-template/-/blob/main/template.yaml[import-existing-api-template/template.yaml^, window="gitlab"] file in GitLab
* *import-existing-app* - Available in the https://gitlab-gitlab.{openshift_cluster_ingress_domain}/pe{team-id}/import-existing-app-template/-/blob/main/template.yaml[import-existing-app-template/template.yaml^, window="gitlab"] file in GitLab
...
. Visit your https://gitlab-gitlab.{openshift_cluster_ingress_domain}/pe{team-id}/developer-hub-config/-/edit/main/values.yaml[rhdh/developer-hub-config repository on GitLab^, window="gitlab"]. If prompted, login with `{pe-username}` / `{common_password}`.
...
catalog:
  locations:
    - type: url
      target: https://gitlab-gitlab.{openshift_cluster_ingress_domain}/pe{team-id}/import-existing-api-template/-/blob/main/template.yaml
    - type: url
      target: https://gitlab-gitlab.{openshift_cluster_ingress_domain}/pe{team-id}/import-existing-app-template/-/blob/main/template.yaml
----
+
...
After you complete the configuration in the following sections and commit your changes, you can verify the templates are available by navigating to the https://backstage-rhdh-{team-id}.{openshift_cluster_ingress_domain}/create[*Create* screen, window="rhdh"]. You should see both templates for importing APIs and applications to {product_name_rhdh}.
...
. From {product_name_rhdh}, navigate to https://backstage-rhdh-{team-id}.{openshift_cluster_ingress_domain}/create[*Create* screen, window="rhdh"]; choose *Register Existing Component* button
. Paste the below URL and click on *Analyze*
+
[source,bash,role=execute,subs="attributes"]
----
https://gitlab-gitlab.{openshift_cluster_ingress_domain}/pe{team-id}/rhdh-entities/-/blob/main/locations.yaml
----
