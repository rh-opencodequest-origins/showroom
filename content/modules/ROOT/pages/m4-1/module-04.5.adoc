...
. Visit your instance of https://backstage-rhdh-team{team-id}.{openshift_cluster_ingress_domain}[{product_name_rhdh}^, window="rhdh"]. If prompted, login as `{dev-username}` / `{common_password}`.
...
. Click on the *< > View Source* button on the {product_name_rhdh} https://backstage-rhdh-team{team-id}.{openshift_cluster_ingress_domain}/catalog/default/component/parasol-store-my-feature-branch[Component Overview page^, window="rhdh"] to access the source code repository.
+
...
. Navigate to `parasol-store-my-feature-branch` component https://backstage-rhdh-team{team-id}.{openshift_cluster_ingress_domain}/catalog/default/component/parasol-store-my-feature-branch/ci[from your {product_name_rhdh}^, window="rhdh"] 
