...
=== Dev Deployment
As a developer, I want to see my deployment: Did my application really deploy to the dev environment?
...
IMPORTANT: Remove the dependency as it's not needed in our code and not compliant to our security rules !
...
NOTE:  The pipeline will run again!
...
You can add `/api/villain/hello`, `/api/villain/random`, or `/api/villain` to the URL in your browser to attack the good endpoints and check your work
...
NOTE: Promotion to preprod and prod will be done for every service. Each team member accomplishes this step!
