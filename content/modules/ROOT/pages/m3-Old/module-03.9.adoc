...
. Access the https://gitlab-gitlab.{openshift_cluster_ingress_domain}/pe{team-id}/developer-hub-config/-/blob/main/values.yaml[rhdh/developer-hub-config configuration on GitLab, window="gitlab"].
...
====
. You will now be able to view the RBAC policies you setup in the https://backstage-rhdh-{team-id}.{openshift_cluster_ingress_domain}/rbac[Administration > RBAC, window="rhdh"] left-hand menu.
...
. Navigate to the https://backstage-rhdh-{team-id}.{openshift_cluster_ingress_domain}/create[*Create* screen, window="rhdh"] and confirm you can see the *Register Existing Component* button. 
...
. Navigate to the https://backstage-rhdh-{team-id}.{openshift_cluster_ingress_domain}/create[*Create* screen, window="rhdh"]. 
