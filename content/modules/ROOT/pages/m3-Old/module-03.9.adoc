...
. Access the https://gitlab-gitlab.{openshift_cluster_ingress_domain}/peteam{team-id}/developer-hub-config/-/blob/main/values.yaml[rhdh/developer-hub-config configuration on GitLab, window="gitlab"].
. Select *Edit > Edit single file*. When prompted, login as `{pe-username}` / `{common_password}`.
...
====
. You will now be able to view the RBAC policies you setup in the https://backstage-rhdh-team{team-id}.{openshift_cluster_ingress_domain}/rbac[Administration > RBAC, window="rhdh"] left-hand menu.
...
user:default/{dev-username},{dev-username} rhdh,{dev-username}@rhdemo.com,"Tue, 10 Dec 2024 05:25:00 GMT"
user:default/{pe-username},{pe-username} rhdh,{pe-username}@rhdemo.com,"Tue, 10 Dec 2024 05:25:22 GMT"
```

. Navigate to the https://backstage-rhdh-team{team-id}.{openshift_cluster_ingress_domain}/create[*Create* screen, window="rhdh"] and confirm you can see the *Register Existing Component* button. 
...
. Navigate to the https://backstage-rhdh-team{team-id}.{openshift_cluster_ingress_domain}/create[*Create* screen, window="rhdh"]. 
