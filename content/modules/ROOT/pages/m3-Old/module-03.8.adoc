...
IMPORTANT: Please make sure to log in as a *Platform Engineer* with `{pe-username}` / `{common_password}`. Expand to the below note to familiarise yourself with the process.
...
. Visit the https://gitlab-gitlab.{openshift_cluster_ingress_domain}/peteam{team-id}/template-quarkus-simple/-/blob/main/skeleton/catalog-info.yaml[rhdh/template-quarkus-simple/skeleton/catalog-info.yaml,  window="gitlab"] file in GitLab.
...
Return to your {product_name_rhdh} instance to view the https://backstage-rhdh-team{team-id}.{openshift_cluster_ingress_domain}/catalog/default/template/quarkus-web-template[quarkus-web-template^, window="rhdh"] after committing the change. Use the *Schedule entity refresh* button to pull this change from Git to {product_name_rhdh} for your Quarkus application. 
