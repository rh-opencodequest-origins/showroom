...
The Orchestrator plugin acts as an interface between {product_name_rhdh} and Serverless Workflow. Serverless Workflow is a cloud-native workflow engine that allows the design, deployment, and execution of arbitrarily complex workflows, which can integrate with external systems through several mechanisms.
...
image:m7/orchestrator-architecture-container-diagram.jpg[Orchestrator architecture container diagram,link=self,window=blank]

As a use case for Developer Self-Service, we are going to look at a pretty simple use case: a Developer can request a namespace on an OpenShift cluster. The namespace has limited resources, defined by resource quota and limit ranges. In our case, the namespaces come in two sizes, small and large. Large namespaces, however, require approval. 

IMPORTANT: Please make sure to log in as a *Platform Engineer* with *`{pe-username}` / `{common_password}`*. Refer to the guide below for assistance.
...
====
+
image:m7/orchestrator-plugin.png[Orchestrator and Notification dynamic plugins, width=60%,link=self,window=blank]
+
...
+
image:m7/orchestrator-plugin-configuration.png[Orchestrator configuration, width=60%,link=self,window=blank]
+
...
. Click the *Refresh* button on the https://openshift-gitops-server-openshift-gitops.{openshift_cluster_ingress_domain}/applications[`RHDH` Application in OpenShift GitOps^, window="argo"].
...
+
image:m7/orchestrator-notification-menu.png[Orchestrator and Notification in RHDH, width=60%,link=self,window=blank]
. Select *Orchestrator* from the left-hand side menu.
+
image:m7/workflow-orchestrator-overview.png[Workflow Orchestrator overview,link=self,window=blank]
+
...
+
image:m7/notifications-overview.png[Notifications overview,link=self,window=blank]
+
...
To illustrate the possibilities of the Orchestrator plugin, let's imagine the following use case: a developer requests a namespace in an OpenShift cluster. Namespaces are limited by resource quota and limits. They come in two sizes, small and large, where the large namespace enables more resources. The large namespace requires approval. +
...
+
image:m3/create-icon.png[Self-service menu with create icon in navigation bar,link=self,window=blank] 
...
+
image:m7/rhdh-register-template.png[Registering Software Templates,link=self,window=blank]
+
...
+
image:m7/rhdh-developer-self-service-templates.png[Developer Self Service Software Templates,link=self,window=blank]

The Software Templates to create namespaces are not meant to be used directly, but rather through an Orchestrator Workflow. So while they are visible (and usable) to members of the Platform Engineering group, they are not accessible to developers.
...
+
image:m7/rhdh-developer-self-service-templates-dev.png[Developer view showing limited Software Templates,link=self,window=blank]
...
image:m7/template-large-namespace-annotation.png[Template Namespace Large Annotation,link=self,window=blank]
...
+
image:m7/gitlab-copy-file-contents.png[GitLab Copy File Contents,link=self,window=blank]
. Visit the https://console-openshift-console.{openshift_cluster_ingress_domain}[OpenShift console^, window="console"] in your OpenShift cluster. If needed, log in as `admin/{common_password}`
. Click the *import* button on the top right menu.
+
image:m7/openshift-console-import.png[OpenShift Console Import,link=self,window=blank]
. Paste the ArgoCD manifest in the editor. Click the *Create* button.
+
image:m7/openshift-console-import-create.png[OpenShift Console Import Create,link=self,window=blank]
...
+
image:m7/openshift-console-orchestrator-namespace.png[OpenShift Pods in orchestrator namespace,link=self,window=blank]
...
+
image:m3/pe-signout.png[Platform Engineer sign out from Developer Hub,width=70%,link=self,window=blank]
====
. Select the *Orchestrator* item in the left-hand menu.
. Notice that the `Create OpenShift Namespace` Workflow is available.
+
image:m7/orchestrator-workflow-deployed.png[Create OpenShift Namespace workflow deployed in Orchestrator,link=self,window=blank]
...
.Click to learn how to log in as a Developer
[%collapsible]
====
...
+
image:m7/orchestrator-select-workflow.png[Select Create OpenShift Namespace workflow,link=self,window=blank]
. Expect to see the workflow definition and a graphical representation of the workflow.
+
image:m7/orchestrator-workflow-overview.png[Workflow definition and graphical representation,link=self,window=blank]
+
...
+
If a large namespace is requested, the request needs to be approved. For simplicity reasons, we use issues in a GitLab repo as a ticketing system (in real life, that would probably be ServiceNow, Jira, or an equivalent system). If the request is approved, the software template is executed, and the user is notified. If the request is denied, the user is notified as well, but no namespace is created.
. Click on the *Run* button at the top right of the screen to start an instance of the workflow.
+
image:m7/orchestrator-workflow-run.png[Run button to start workflow instance,link=self,window=blank]
...
+
image:m7/orchestrator-workflow-form.png[Workflow form for namespace request details,link=self,window=blank]
. Click on *Next*. Expect to see an overview of your request.
. Click on *Run* to start the workflow execution.
+
image:m7/orchestrator-workflow-run-2.png[Workflow request overview before Run,link=self,window=blank]
. After a few seconds, the status of the workflow moves to *Run completed*.
+
image:m7/orchestrator-workflow-run-completed.png[Workflow status Run completed,link=self,window=blank]
...
+
image:m7/notification-1.png[Notification that namespace was created,link=self,window=blank]
...
+
image:m7/orchestrator-workflow-run-3.png[Run icon next to workflow name,link=self,window=blank]
...
. Click on the link in the notification. This opens the issue in GitLab.
+
image:m7/gitlab-issue-1.png[GitLab issue created for namespace request,link=self,window=blank]
...
+
.. Make sure you are logged in to GitLab as *`{pe-username}` / `{common_password}`*.
.. Click on the *Edit* button next to *Labels* on the right.
+
image:m7/gitlab-issue-2.png[Edit button next to Labels in GitLab issue,link=self,window=blank]
.. Select the *Approved* label.
+
image:m7/gitlab-issue-3.png[Select Approved label in GitLab issue,link=self,window=blank]
.. To actually add the label, you need to click on a different area of the page, outside of the *Labels* area.
+
image:m7/gitlab-issue-4.png[Add label by clicking outside Labels area,link=self,window=blank]
...
+
image:m7/notification-2.png[Notification that large namespace was created,link=self,window=blank]
+
The workflow instance checks the status of the GitLab issue at a regular interval (every 60 sec. in this case). When the workflow detects the *Approved* label on the issue, it proceeds to create the namespace and closes the ticket afterwards.
...
+
image:m7/gitlab-issue-5.png[GitLab issue closed by workflow,link=self,window=blank]
