...
. Visit your https://gitlab-gitlab.{openshift_cluster_ingress_domain}/peteam{team-id}/developer-hub-config/[rhdh/developer-hub-config repository on GitLab, window="gitlab"].
...
. After a few moments your new {product_name_rhdh} configuration will finish rolling out. Visit your https://backstage-rhdh-team{team-id}.{openshift_cluster_ingress_domain}/[instance of {product_name_rhdh}^, window="rhdh"] and view the Home page. Note two new menu items in the left-hand side menu, *Orchestrator* and *Notifications*.
+
...
The Software Templates for Developer Self Service are referenced in the _location.yaml_ file in the https://gitlab-gitlab.{openshift_cluster_ingress_domain}/peteam{team-id}/template-developer-self-service[rhdh/template-developer-self-service, window="gitlab"] repository in GitLab.
...
----
https://gitlab-gitlab.{openshift_cluster_ingress_domain}/peteam{team-id}/template-developer-self-service/-/blob/main/location.yaml?ref_type=heads
----
...
. Open the https://gitlab-gitlab.{openshift_cluster_ingress_domain}/peteam{team-id}/orchestrator-workflows/-/blob/main/manifests/create-ocp-namespace-swt/application.yaml?ref_type=heads[ArgoCD Application manifest in GitLab^, window="gitlab"]
...
. Visit your https://backstage-rhdh-team{team-id}.{openshift_cluster_ingress_domain}/[instance of {product_name_rhdh}^, window="rhdh"]. Ensure you're logged in as a Platform Engineer.
+
.Click to see how
[%collapsible]
====
* Navigate to {product_name_rhdh}'s https://backstage-rhdh-team{team-id}.{openshift_cluster_ingress_domain}/settings[Settings, window="rhdh"] screen and check the logged-in user's name under the Profile section. 
...
+
Also notice that the https://gitlab-gitlab.{openshift_cluster_ingress_domain}/peteam{team-id}/rhdh-issues/-/issues/1[Issue in GitLab^, window="gitlab"] has been closed by the workflow.
+
