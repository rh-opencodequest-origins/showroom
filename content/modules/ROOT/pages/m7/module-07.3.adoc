...
. Select *Edit > Edit single file*. Login as a Platform Engineer with `{pe-username}` / `{common_password}` when prompted.
...
. Navigate to view the https://openshift-gitops-server-openshift-gitops.{openshift_cluster_ingress_domain}/applications[`backstage` Argo CD Application in OpenShift GitOps^, window="argo"]. Login as `admin/{common_password}` if prompted.
