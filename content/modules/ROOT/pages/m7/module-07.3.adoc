...
. Select the *Create...* item in the left-hand menu to see the new templates.
+
...
. Select the *Create...* item on the left-hand side menu.
...
+
Also notice that the https://gitlab-gitlab.{openshift_cluster_ingress_domain}/rhdh/rhdh-issues/-/issues/1[Issue in GitLab] has been closed by the workflow.
+
