...
The Adoption Insights plugin provides a dashboard that displays the number of users, the number of active users, and the number of new users. It also provides a list of the most popular components and systems in your catalog.

To enable the Adoption Insights plugin, you need to add the following configuration in your https://gitlab-gitlab.{openshift_cluster_ingress_domain}/rhdh/developer-hub-config/-/blob/main/values.yaml[values.yaml file in the developer-hub-config repository^, window="gitlab"].

1. Uncomment the `--- ADOPTION_INSIGHTS_PLUGIN ---` sections in the `dynamic.plugins` section of the YAML file.
+
image:m7/adoption-insights-plugin-commented-block.png[width=100%]

2. Uncomment the `--- ADOPTION_INSIGHTS_PLUGIN ---` sections in the `upstream.appConfig` section of the YAML file.
+
image:m7/adoption-insights-plugin-config-commented-block.png[width=50%]

3. Scroll down and enter a commit message `feat: enable adoption insights plugin`.

4. Click the *Commit changes* button.

5. Click the *Refresh* button on the https://openshift-gitops-server-openshift-gitops.{openshift_cluster_ingress_domain}/applications[`backstage` Application in OpenShift GitOps^, window="argo"].

Your changes will be rolled out to the `backstage` Argo CD application. Once the application is in a *Healthy* state, you can view the Adoption Insights plugin by navigating to the `Administration -> Adoption Insights` menu in your {product_name_rhdh} instance.

image:m7/adoption-insights-plugin-dashboard.png[width=100%]

Nice work! You can check the adoption of your {product_name_rhdh} instance.
