:imagesdir: ../../assets/images

= Module 6: Observability: Security

include::../style.adoc[]



== Objectives

* SecOps is alerted about a compromised library
* PE runs a vulnerability scan to detect which applications are affected, and who to contact.
* Affected teams are alerted and need to issue fixes asap.
* PE checks existing software templates for the detected vulnerabilities.
* Changes are rolled out to the RHDH Software Template catalogue if needed
* ( Policy Exception request for a low-severity issue may be approved by SecOps)



== Your lab environment

== Introductions to Concepts


== Activity 1 


== Activity 2

== Conclusion
