...
Let's explore the Template structure using a sample Template in the https://gitlab-gitlab.{openshift_cluster_ingress_domain}/pe{team-id}/template-quarkus-simple[rhdh/template-quarkus-simple^, window="gitlab"] repository in GitLab. 
...
Notice how the `parameters` are referenced in the `steps`? Another point of note is that a `user` variable is available to access data related to the user who's using the Template, and subsequent `steps` can access `output` from prior steps. 

The `output` values are documented on a per-plugin basis. You can find the values for the specific version of your installed plugins by accessing the https://backstage-rhdh-{team-id}.{openshift_cluster_ingress_domain}/create/actions[/create/actions^, window="rhdh"] endpoint on your {product_name_rhdh} instance.

== Output

The _spec.output_ can use the outputs from the `steps` to display useful information such as: 
