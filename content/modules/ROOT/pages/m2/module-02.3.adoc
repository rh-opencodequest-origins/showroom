...
[NOTE]
====
You may need to logout to see the title change. Click the Guest user icon in the top right corner and select *Sign out*. Then log back in to the https://backstage-backstage.{openshift_cluster_ingress_domain}[{product_name_rhdh} instance, window="rhdh"]  as a Platform Engineer with `{pe-username}` / `{common_password}`.
