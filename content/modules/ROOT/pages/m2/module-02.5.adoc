...
In this activity, you'll configure an OpenID Connect authentication provider.
This will enable developers within your organization to log in using their single sign-on (SSO) credentials.
Remember, we have imported users and groups from *Keycloak*. The good news is Keycloak will provide authentication to Red Hat Developer Hub. 
...
. Confirm the commit has triggered a Red Hat Developer Hub rollout https://openshift-gitops-server-openshift-gitops.{openshift_cluster_ingress_domain}/applications/rhdh-{team-id}[ Red Hat Developer Application in OpenShift GitOps^, window="argo"]. If prompted, log in as `{pe-username}/{common_password}`.
...
[WARNING]
====
If the application is healthy has started but there is no change on Developer Hub UI
====
.Get help here
[%collapsible]
====
