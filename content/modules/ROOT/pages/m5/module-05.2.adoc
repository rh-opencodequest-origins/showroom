...
* On {product_name_rhdh}, in the `parasol-store` component's https://backstage-rhdh-team{team-id}.{openshift_cluster_ingress_domain}/catalog/default/component/parasol-store/pr[Pull/Merge Requests page, window="rhdh"], you can view the a list of Gitlab Merge Request Statuses.
...
* Navigate to the {product_name_rhdh} https://backstage-rhdh-team{team-id}.{openshift_cluster_ingress_domain}/catalog/default/component/parasol-store[`parasol-store` component, window="rhdh"]. From the *CD* tab, access the `parasol-store-prod` Argo CD app by clicking on the arrow.
+
