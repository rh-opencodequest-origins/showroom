...
. Navigate to the https://backstage-rhdh-team{team-id}.{openshift_cluster_ingress_domain}/catalog/default/component/parasol-store-my-feature-branch[feature branch component, window="rhdh"] you had created. If prompted, login as `{dev-username}` / `{common_password}`.
...
. Since this merge request is to the `parasol-store` Component, the `parasol-store-build-pr-open` pipeline gets triggered and can be viewed in the https://backstage-rhdh-team{team-id}.{openshift_cluster_ingress_domain}/catalog/default/component/parasol-store/ci[`parasol-store` component, window="rhdh"].
+
...
. The `Merge` action triggers another Pipeline in the https://backstage-rhdh-team{team-id}.{openshift_cluster_ingress_domain}/catalog/default/component/parasol-store/ci[`parasol-store` component, window="rhdh"] on {product_name_rhdh}, which will update the Argo CD manifests with the new image tag.
+
...
. In {product_name_rhdh}, access the CD tab of the https://backstage-rhdh-team{team-id}.{openshift_cluster_ingress_domain}/catalog/default/component/parasol-store/cd[parasol-store component^, window="rhdh"].
+
