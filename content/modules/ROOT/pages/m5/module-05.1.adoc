...
== Developer Activity: Create a Merge Request from the feature branch

include::../style.adoc[]


. Navigate to the https://backstage-rhdh-{team-id}.{openshift_cluster_ingress_domain}/catalog/default/component/parasol-store-my-feature-branch[feature branch component, window="rhdh"] you had created. If prompted, log in as `{dev-username}` / `{common_password}`.
...
. Since this merge request is to the `parasol-store` Component, the `parasol-store-build-pr-open` pipeline gets triggered and can be viewed in the https://backstage-rhdh-{team-id}.{openshift_cluster_ingress_domain}/catalog/default/component/parasol-store/ci[`parasol-store` component, window="rhdh"].
+
...
. In GitLab, navigate to https://gitlab-gitlab.{openshift_cluster_ingress_domain}/parasol/parasol-store/-/hooks[parasol-store's Webhooks page, window="gitlab"] to view the Webhooks that have been set up as part of this workshop.
====


== Developer Activity: Merge the Merge Request from the feature branch
...
. The `Merge` action triggers another Pipeline in the https://backstage-rhdh-{team-id}.{openshift_cluster_ingress_domain}/catalog/default/component/parasol-store/ci[`parasol-store` component, window="rhdh"] on {product_name_rhdh}, which will update the Argo CD manifests with the new image tag.
+
...
. In {product_name_rhdh}, access the CD tab of the https://backstage-rhdh-{team-id}.{openshift_cluster_ingress_domain}/catalog/default/component/parasol-store/cd[parasol-store component^, window="rhdh"].
+
image:m5/parasol-store-argo.png[]
. Click on the outgoing-arrow icon next to *parasol-store-dev* to access the *rhdh-gitops* instance of Argo CD that's used to manage deployed applications. Log in as `admin` / `{common_password}`.
+
