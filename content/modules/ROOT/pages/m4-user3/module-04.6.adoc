...
IMPORTANT: Please make sure you are logged in as a Developer with `{dev-username}` / `{common_password}` as you were guided to in the previous step. We advice you to use a *private / incognito window* to be logged out properly !  
...
As you can see, it looks like a **bad dependancy**  has introduced a bad dependency in our code ! 
...
// how to test fight service with terminal ?  
#You can add `/api/villain/hello` , `/api/heroes/random` or `/api/villaib` to the URL in you browser to attack the good endpoints and check your work#
...
#Promotion to preprod and prod should be done for every services fight#
